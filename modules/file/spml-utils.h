 ...
 */ 
typedef enum {
    UNKNOWN_DATAFORMAT, FLOAT32, FLOAT64, INT8, INT16, INT32, UINT8, UINT16, UINT32, STRING
} dataFormat;
...
*/
int inflate_init(zlib_stream *zstr) {
	int ret;
...
*/
int inflate_set_in_buffer(zlib_stream *zstr, char *in_buf, int count) {
	if (count > CHUNK) {
 ...
 */
int inflate_get_out_buffer(zlib_stream *zstr, GArray** out_buf) {
	int count;
	int ret;
   /* run inflate() on input until output buffer not full */
    do {
		...
		*out_buf = g_array_append_vals(*out_buf, zstr->out, count);
		if (*out_buf == NULL) {
			g_warning("Zlib inflate: output buffer wasn't written to dynamic array.\n");
			return Z_ERRNO;
		}
...
*/
void inflate_destroy(zlib_stream *zstr) {
    ...
    *out_buf = g_array_new(FALSE, FALSE, sizeof(char));
    for (i = 0; (i + CHUNK) < in_buf->len; i += CHUNK) {
		...
		if (inflate_get_out_buffer(&zstr, out_buf) != Z_OK) {
			g_warning("Cannot inflate zlib compression. Be sure it is a compressed stream.\n");
			ret = -1;
			break;
		}
    }
...
char convert_b64_symbol_to_number(char ch) {
    if (ch >= 'A' && ch <= 'Z') {
        return ch - 'A';
    } else if (ch >= 'a' && ch <= 'z') {
        return ch - 'a' + 26;
        ...
        g_warning(" Cannot translate symbol '%c' to number in BASE64 encoding.", ch);
        return 0;
    }
...
{ 
    unsigned char i[4];
    if (in[0] == '=' || in[1] == '=') return 0; // first or second byte can't be '='
    ...
    *out_buf = g_array_new(FALSE, FALSE, sizeof(char));
    for (i=0; i< len; i++) {
        ...
        if ((pos % 4) == 0) {
            ...
            if (valid_bytes > 0) {
                *out_buf = g_array_append_vals(*out_buf, &b64_dec_chars, valid_bytes);
                if (*out_buf == NULL) {
                    return -1;
                }
            }
