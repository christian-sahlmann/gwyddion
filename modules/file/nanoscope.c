    ...
    if (ok) {
        ...
        for (l = list; l; l = g_list_next(l)) {
            ...
            if (ndata->data_field) {
                ...
                if (data_field_has_highly_nosquare_samples(ndata->data_field)) {
                    g_snprintf(key, sizeof(key), "/%d/data/realsquare", i);
                    gwy_container_set_boolean_by_name(container, key, TRUE);
                }
