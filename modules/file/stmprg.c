...
static gint        stmprg_detect             (const gchar *filename,
					      gboolean only_name);
...
static gint
stmprg_detect(const gchar *filename, 
	       gboolean only_name)
{
   gint score = 0;
   FILE *fh;
...
float ByteSwapFloat(float x)
{
   ByteSwap(x);
   return x;
}

#define GFLOAT_FROM_BE(val) (G_BYTE_ORDER == G_LITTLE_ENDIAN ? ByteSwapFloat(val) : (val))
...
#define STORE(format, keystr, val) \
key = g_strdup_printf("/meta/%s", keystr); \
value = g_strdup_printf(format, val); \
gwy_debug("key = %s, val = %s\n", key, value); \
gwy_container_set_string_by_name(data, key, value); \
g_free(key)
  
#define STORE_F(keystr, valf) STORE("%f", keystr, valf)
#define STORE_S(keystr, vals) STORE("%s", keystr, vals)
#define STORE_I(keystr, vali) STORE("%i", keystr, vali)
  ...
  store_metadata(GwyContainer *data)
{
   ...
   STORE_F("inc_x", mainfield.inc_x);
   STORE_F("inc_y", mainfield.inc_y);
   STORE_F("angle", mainfield.angle);
   STORE_F("sol_z", mainfield.sol_z);
   STORE_F("voltage", control.voltage);
   STORE_F("current", control.current);
   STORE_I("point_time", control.point_time);
   STORE_S("date", other_ctrl.date);
   STORE_S("comment", other_ctrl.comment);
   STORE_S("username", other_ctrl.username);
}
 ...
 */
static gboolean
  read_binary_ubedata(gint n, gdouble *data,
		   guchar *buffer,
		   gint bpp)
{
   gint i;
