...
gwy_data_field_fractal_partitioning(GwyDataField *data_field, GwyDataLine *xresult, GwyDataLine *yresult, GwyInterpolationType interpolation)
{
    ...
    {
        rp = ROUND(pow(2,l));
        for (i=0; i<((buffer->xres-1)/rp-1); i++)            //        for(i=(ROUND(pow(2,dimexp)/pow(2,l+1))-1); i>=0; i--)
        {
            for (j=0; j<((buffer->yres-1)/rp-1); j++)                       // for (j=(ROUND(pow(2,dimexp)/pow(2,l+1))-1); j>=0; j--)
            {
                rms = gwy_data_field_get_area_rms(buffer, i*(rp), j*(rp), (i+1)*(rp)+1, (j+1)*(rp)+1);
...
gdouble
gwy_data_field_fractal_cubecounting_dim(GwyDataLine *xresult, GwyDataLine *yresult, gdouble *a, gdouble *b)
{     
...
gdouble
gwy_data_field_fractal_triangulation_dim(GwyDataLine *xresult, GwyDataLine *yresult, gdouble *a, gdouble *b)
{     
    gwy_data_field_fractal_fit(xresult, yresult, a, b);
...
gdouble
gwy_data_field_fractal_partitioning_dim(GwyDataLine *xresult, GwyDataLine *yresult, gdouble *a, gdouble *b)
{     
    gwy_data_field_fractal_fit(xresult, yresult, a, b);
...
gdouble
gwy_data_field_fractal_psdf_dim(GwyDataLine *xresult, GwyDataLine *yresult, gdouble *a, gdouble *b)
{     
    gwy_data_field_fractal_fit(xresult, yresult, a, b);
