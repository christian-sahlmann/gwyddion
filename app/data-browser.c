 ...
 * source files.  And indeed it has managed to gather lots of it.  Part of
 * it has been offloaded to data-browser-aux.c. */
#include "config.h"
#include <stdlib.h>
#include <string.h>
#include <stdarg.h>
    ...
    if (gtk_tree_model_get_iter_first(model, &iter)) {
        do {
            ...
            if ((item = gwy_app_data_proxy_get_3d(proxy, id))) {
                ...
                _gwy_app_update_3d_window_title(GWY_3D_WINDOW(assoc->object),
                                                id);
            }
        } while (gtk_tree_model_iter_next(model, &iter));
    }
                                ...
                                gint id)
{
    ...
    gwy_app_data_browser_set_file_present(browser, TRUE);
    gtk_widget_show_all(data_window);
    _gwy_app_update_surface_info(proxy->container,
                                 id, GWY_DATA_VIEW(data_view));
